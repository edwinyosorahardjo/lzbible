/*
LZ Bible - partially loaded LZW compressed bible
Depedency:
    Lz-String
    JQuery
*/

var LzBible = function(){
    'use strict';
    var version  = '0.0.1';
    var initialized;
    var baseUrl;
    ...
    function getBooks(bibleParam,callback){
        ...
        var booksPath = bible.defaultIndex?('defaults/books/' + bible.defaultIndex + '.txt') :(bible.dbpath + '/' + 'books.txt');

        $.when(get(booksPath)).then(d =>{ 
            dfd.resolve(books = d);
        });
        return dfd.promise();
    }
    ...
    function getPassages(book, chapter){
        var dfd = $.Deferred();
        if (model.bible.minBookIdx && book < model.bible.minBookIdx)
            console.log('minimum book idx = ' + model.bible.minBookIdx);

        $.when(get(bible.dbpath + '/' + book + '_' + chapter + '.txt',null,null)).then(
        d =>{ 
