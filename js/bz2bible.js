/*
BZip2 Bible - partially loaded BZip2 compressed bible
Depedency:
    BZip2 Decoder
    JQuery
*/

var Bz2Bible = function () {
    'use strict';
    var version = '0.0.1';
    var initialized;
    var baseUrl;
    ...
    var bookData = [[]]; // [Chapter[VerseTexts]]
    ...
    function loadAvailableBibles() {
        ...
        //});
        $.when(getBzData(baseUrl + 'bibles.bz2')).then(d => {
            dfd.resolve(bibles = d);
        });
        return dfd.promise();
    }
    ...
    function get(url, success, error, compression) {
        var dfd = $.Deferred();
        $.ajax({
            url: baseUrl + url,
            crossDomain: true,
            cache: true,
            success: d => {
                dfd.resolve(JSON.parse(LZString.decompressFromBase64(d)));
            }
        });
        return dfd.promise();
    }

    // load array of [Chapters][Verse]
    function getBookData(bookIdx) {
        if (model.bible.minBookIdx && bookIdx < model.bible.minBookIdx-1)
            console.log('minimum book idx = ' + model.bible.minBookIdx);

        var dfd = $.Deferred();
        var dataPath = baseUrl + bible.dbpath + '/bz2/' + (bookIdx + 1) + '.bz2';
        $.when(getBzData(dataPath)).then(d => {
            //console.log(d);
            dfd.resolve(bookData = d);
        });
        return dfd.promise();
    }

    // Get and decode BZip Data
